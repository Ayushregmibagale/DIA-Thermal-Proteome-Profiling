...
# CETSA-MS SILAC version with safer normalization
#
# Updated normalization logic
# ---------------------------
# 1. Precursor.Quantity is run-level total-signal normalized.
#    This is used for LFQ/intensity-style protein abundance outputs.
#
# 2. Precursor.Translated is NOT total-sum normalized before H/L ratio calculation.
#    For SILAC CETSA-MS, H/L is an intra-run ratio and should preserve the DIA-NN translated
#    channel relationship. Total-sum normalizing translated values can distort real thermal
#    changes in soluble protein abundance.
#
# 3. Protein-level log2(H/L) ratios are median-centred per Run.
...
# LFQ_H               = Heavy/drug protein abundance from normalized Precursor.Quantity
# LFQ_L               = Light/DMSO protein abundance from normalized Precursor.Quantity
#
#############################################################################################################
...
# Clean names: remove dashes, flag and strip trailing timestamp of 14 digits
report[, Run := gsub("-", "_", Run)]
report[, Had_timestamp := grepl("_[0-9]{14}$", Run)]
report[, Run_clean := sub("_[0-9]{14}$", "", Run)]

# Extract temperature from names such as E260328_06_CET_37_1
...
# Flag runs where original replicate suffix differs from corrected, or timestamp was stripped
runs_dt[, Needs_rename := (Replicate_old != Replicate_num) | Had_timestamp]

# Join derived columns back to full report
...
#############################################################################################################
# QUANTITY CLEANUP AND NORMALIZATION
#############################################################################################################
...
# Convert zeroes to NA
report[Precursor.Quantity == 0, Precursor.Quantity := NA_real_]
report[Precursor.Translated == 0, Precursor.Translated := NA_real_]

# Save raw values before any normalization
report[, Precursor.Quantity.raw := Precursor.Quantity]
report[, Precursor.Translated.raw := Precursor.Translated]

# Remove precursors not quantified at all by Precursor.Quantity
report <- report[!is.na(Precursor.Quantity)]

# Run-level total-signal normalization for Precursor.Quantity only.
# This normalized quantity is used for MaxLFQ and intensity outputs.
# Do NOT apply this to Precursor.Translated.raw, because H/L ratio should preserve the SILAC channel relationship.
run_sums <- report[, .(sumQ = sum(Precursor.Quantity, na.rm = TRUE)), by = Run]
target_sum <- run_sums[, median(sumQ[is.finite(sumQ) & sumQ > 0], na.rm = TRUE)]

if (!is.finite(target_sum) || target_sum <= 0) {
  stop("target_sum is invalid. Check Precursor.Quantity values.")
}

report[, run_sumQ := sum(Precursor.Quantity, na.rm = TRUE), by = Run]
report[is.finite(run_sumQ) & run_sumQ > 0,
       Precursor.Quantity := Precursor.Quantity / run_sumQ * target_sum]
report[, run_sumQ := NULL]

message("Precursor.Quantity was total-signal normalized per Run.")
message("Precursor.Translated.raw was kept unnormalized for SILAC H/L ratio calculation.")
...
report[grepl("SILAC-[A-Z]-L", Precursor.Id), label := "L"]  # Light = DMSO/control
report[grepl("SILAC-[A-Z]-H", Precursor.Id), label := "H"]  # Heavy = drug/treated
...
#############################################################################################################
# INTRA-RUN SILAC H/L RATIOS
#############################################################################################################

# H/L ratio uses raw DIA-NN translated values, not total-sum-normalized values.
SILAC <- dcast(
  report[!is.na(label)],
...
#############################################################################################################
# MAXLFQ ON NORMALIZED PRECURSOR.QUANTITY
#############################################################################################################
...
LFQ_fun <- function(dt, LFQ_colname) {
  ...
  tmp <- dt[
    is.finite(Precursor.Quantity) & Precursor.Quantity > 0,
    .(
  ...
  if (nrow(tmp) == 0) {
    warning(sprintf("No finite quantities for %s", LFQ_colname))
    return(data.table(Protein.Group = character(), Run = character()))
  }
...
# The run median is expected to be near 0 because most proteins should not change.
# Subtracting it corrects SILAC mixing/channel bias.
if ("log2_HL_ratio_raw" %in% names(proteins)) {
  ...
  # Backward-compatible column name. Downstream scripts that expect log2_HL_ratio will now get normalized values.
...
# These are useful for quick inspection only. Your full CETSA stats script can redo this more carefully.
# Each protein/replicate/channel is normalized to its own lowest-temperature abundance.
if (all(c("LFQ_L", "LFQ_H", "Temp", "Replicate") %in% names(proteins))) {
  ...
  proteins[, fraction_L := fifelse(is.finite(LFQ_L) & is.finite(LFQ_L_baseline) & LFQ_L_baseline > 0,
                                   LFQ_L / LFQ_L_baseline, NA_real_)]
  proteins[, fraction_H := fifelse(is.finite(LFQ_H) & is.finite(LFQ_H_baseline) & LFQ_H_baseline > 0,
...
# CETSA-MS SILAC design:
# Light L = DMSO/control
# Heavy H = drug/treated
# Each run = one temperature point and one replicate

# Keep numeric temperature for calculations
...
precursor_by_sample <- report[
  !is.na(Precursor.Quantity) & !is.na(label) & !is.na(Temp_numeric),
...
#############################################################################################################
# QC PLOT 2: CUMULATIVE INTENSITY DISTRIBUTIONS
#############################################################################################################
...
pPCA_L <- run_pca_plot(proteins, "LFQ_L", "DMSO control (Light)", temp_color_mapping)
pPCA_H <- run_pca_plot(proteins, "LFQ_H", "Drug treated (Heavy)", temp_color_mapping)
...
#############################################################################################################
# QC PLOT 9: FRACTION NON-DENATURED DISTRIBUTIONS
#############################################################################################################
...
pFrac <- ggplot(fraction_long[is.finite(Fraction) & Fraction > 0],
  ...
  labs(x = "Temperature (°C)", y = "log2 fraction relative to lowest temperature",
       title = "Protein melt-curve normalization check") +
...
message("Use LFQ_L/LFQ_H or fraction_L/fraction_H as secondary melt-curve evidence.")
