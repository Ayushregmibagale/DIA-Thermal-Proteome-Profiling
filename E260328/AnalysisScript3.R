#############################################################################################################
#
# CETSA-MS SILAC ANALYSIS SCRIPT v3.0
#
# DESIGN
# ------
# Input : protein_quant.csv.gz from the updated DIA-NN SILAC protein summarisation script v3
#
# Channel meaning inherited from upstream script:
#   LFQ_L / label L = light = DMSO / vehicle / control
#   LFQ_H / label H = heavy = drug / treated
#
# Important upstream processing
# -----------------------------
#   1. Precursor.Quantity.raw is used upstream for MaxLFQ and intensity outputs.
#      Therefore LFQ_L and LFQ_H in protein_quant.csv.gz are raw-quantity-derived.
#      They are NOT run-level total-signal normalized.
#
#   2. Precursor.Quantity.run_norm exists upstream only for QC.
#      It is NOT used here.
#
#   3. Precursor.Translated.raw is used upstream for SILAC H/L ratio calculation.
#      Protein-level log2_HL_ratio_raw is then run-median centred to produce log2_HL_ratio_norm.
#
#   4. In the updated protein-quant script:
#          log2_HL_ratio = log2_HL_ratio_norm
#      Therefore this analysis script treats log2_HL_ratio as the normalized primary SILAC ratio.
#
...
#           Uses raw-derived LFQ_L and LFQ_H.
#           This script does NOT apply global sample-median normalization by default.
#           It only converts each protein/replicate/condition into fraction remaining:
#               fraction(T) = LFQ(T) / LFQ(lowest temperature)
#
# KEY v3.0 CHANGES
# ----------------
# 1. Updated to match protein-quant v3, where LFQ_L/LFQ_H are raw Precursor.Quantity-derived.
# 2. Global sample median normalization remains OFF by default.
# 3. Track A remains the primary ranking evidence.
# 4. Track B deltaTm and deltaAUC are kept as secondary/supportive evidence.
# 5. Restores the full hit_table construction before Track A-first ranking.
#
#############################################################################################################
...
output_dir  <- "CETSA_SILAC_output_v3_rawLFQ"
...
# Leave FALSE. LFQ_L/LFQ_H are raw-derived and should not be globally rescaled by run/sample.
...
msg("Track B LFQ source  : raw Precursor.Quantity-derived LFQ_L/LFQ_H")
...
if (has_HL_ratio) {
  ...
  ratio_peak_summary <- ratio_temp_summary[, {
    ...
    if (!any(ok)) {
      ...
      temps <- Temp[ok]
      local_idx <- which.max(abs(vals))
      signed_peak <- vals[local_idx]
      .(
        ...
        temp_of_max_abs_log2HL = temps[local_idx],
        auc_log2_HL_curve = safe_auc(temps, vals),
        p_value_ratio_track = NA_real_
      )
    }
...
#############################################################################################################
# TRACK B: RAW-LFQ ABSOLUTE MELT CURVE TRACK, SECONDARY
#############################################################################################################
...
if (do_global_sample_median_normalization) {
  ...
  msg("WARNING: this is not recommended for the main raw-LFQ CETSA analysis.")
  ...
  msg("Skipping global sample median normalization in Track B. Using raw-derived LFQ values.")
}
...
curve_median <- long[, .(
  ...
  q25_frac    = quantile(frac, 0.25, na.rm = TRUE),
  q75_frac    = quantile(frac, 0.75, na.rm = TRUE),
  n_rep       = safe_n(frac)
), by = .(Protein_group, Genes, condition, Temp)]
...
for (cn in c(
  ...
  "deltaTm_mean", "deltaTm_mean_curve", "deltaTm_p_value",
  "max_abs_mean_log2_ratio_trackB", "signed_peak_log2_ratio_trackB"
)) {
...
hit_table[, stabilization_direction := fcase(
  ...
  is.finite(signed_peak_log2_ratio_trackB) & signed_peak_log2_ratio_trackB > 0, "stabilization",
  is.finite(signed_peak_log2_ratio_trackB) & signed_peak_log2_ratio_trackB < 0, "destabilization",
  default = NA_character_
)]

# Primary p-value prefers Track A when available. Store minimum separately.
hit_table[, p_value_best := NA_real_]
hit_table[, p_value_source := NA_character_]

hit_table[is.finite(p_value_ratio_track), `:=`(
  p_value_best = p_value_ratio_track,
  p_value_source = "Track A: SILAC ratio"
)]

hit_table[!is.finite(p_value_best) & is.finite(deltaTm_p_value), `:=`(
...
setorder(hit_table, -sort_has_trackA, -sort_trackA_effect, sort_trackA_p, -sort_deltaTm_rep, -sort_deltaTm_mean)
hit_table[, c("sort_has_trackA", "sort_trackA_effect", "sort_trackA_p", "sort_deltaTm_rep", "sort_deltaTm_mean") := NULL]
...
p_qc_box <- ggplot(
  ...
  labs(
    ...
    y = "log2 raw-derived LFQ abundance",
    title = "Track B abundance distributions, raw-derived LFQ; no global sample normalization"
  ) +
...
p_qc_frac <- ggplot(
  ...
  theme_bw(base_size = 10) +
  labs(x = "Temperature", y = "Fraction vs lowest temperature", title = "Track B raw-LFQ CETSA fractions")
...
make_pca_plot <- function(dt_long, cond_name) {
  ...
  ggplot(pca_dt, aes(PC1, PC2, color = Temp, shape = Replicate)) +
    ...
    theme_bw(base_size = 10) +
    labs(
      title = paste0("PCA - ", cond_name, " raw-LFQ fractions"),
...
if (nrow(heat_source) > 2) {
  ...
  if (nrow(heat_mat) > 2) {
    ...
    if (nrow(heat_mat_top) > 2) {
      png(file.path(output_dir, "Heatmap_top_mean_log2_Drug_vs_DMSO.png"), width = 1800, height = 2200, res = 200)
      pheatmap(
        heat_mat_top,
        scale = "none",
        cluster_rows = TRUE,
...
p_rank <- ggplot(
  ...
  labs(
    title = "Track B supportive raw-LFQ protein-level CETSA shift",
...
plot_one_protein <- function(protein_id = NULL, gene_name = NULL) {
  ...
  p_abs <- ggplot() +
    ...
    geom_ribbon(
      data = dt_med[is.finite(median_frac)],
      aes(Temp, ymin = q25_frac, ymax = q75_frac, fill = condition),
    ...
    geom_point(data = dt_mean[is.finite(mean_frac)], aes(Temp, mean_frac, colour = condition), size = 2.7) +
    geom_line(data = dt_mean[is.finite(mean_frac)], aes(Temp, mean_frac, colour = condition), linewidth = 1.1) +
    theme_bw(base_size = 11) +
    coord_cartesian(ylim = c(0, max_fraction_cap)) +
    labs(
      x = "Temperature (°C)",
...
fwrite(curve_median,        file.path(output_dir, "median_curves.tsv.gz"), sep = "\t")
...
show_cols <- intersect(
  c(
    "Genes", "Protein_group", "primary_evidence", "stabilization_direction",
    "signed_peak_log2_HL", "max_abs_mean_log2_HL", "temp_of_max_abs_log2HL",
    "p_value_ratio_track", "deltaTm_mean", "deltaTm_mean_curve",
    "deltaAUC_mean", "p_value_best", "p_value_source", "n_replicates_present"
  ),
  names(hit_table)
)
