#############################################################################################################
#
# CETSA-MS SILAC TARGET INSPECTION SCRIPT v3.0
#
# PURPOSE
# -------
...
#     - mean_curves.tsv.gz
#     - median_curves.tsv.gz, optional but preferred for cleaner plots
#     - fit_predictions_mean_curves.tsv.gz, optional
#     - deltaTm_mean / deltaTm_mean_curve
#
# IMPORTANT INTERPRETATION
# ------------------------
# Track B curves may look noisier than older globally-normalized curves because LFQ_L/LFQ_H
# are now raw Precursor.Quantity-derived. That is intentional: the curves no longer use
# run-level total-signal normalization, which could artificially force high-temperature
# samples upward.
#
...
# Leave NULL to auto-detect, or set explicitly, e.g.:
...
# Plot styling for cleaner raw-LFQ Track B curves.
show_replicate_lines <- TRUE
show_median_ribbon   <- TRUE
show_fit_curve       <- TRUE
...
# Keep your full target list here.
# The list below is the same target format as before. Replace/edit as needed.
targets <- c(
...
for (f in required_files) {
  if (!file.exists(file.path(input_dir, f))) stop("Required file missing: ", file.path(input_dir, f))
}
...
median_dt     <- read_if_exists("median_curves.tsv.gz")
...
has_median_curves <- nrow(median_dt) > 0
...
if (!("p_value_source" %in% names(hit_dt))) hit_dt[, p_value_source := NA_character_]

msg(if (has_track_a) "Track A SILAC ratio data found." else "Track A data not found; Track B only.")
msg(if (has_median_curves) "Median Track B curves found; using ribbons in target plots." else "Median curves not found; target plots will use mean curves only.")
...
target_summary <- merge(matched_primary, hit_dt, by = c("Protein_group", "Genes"), all.x = TRUE)
...
plot_one_target <- function(protein_group_id, target_label = NULL) {
  ...
  dt_med   <- if (has_median_curves) median_dt[Protein_group == protein_group_id] else data.table()
  ...
  subtxt <- paste0(
    ...
    "\nTrack B raw-LFQ deltaTm=", fmt(dTm),
  ...
  if (show_median_ribbon && nrow(dt_med) > 0 && all(c("median_frac", "q25_frac", "q75_frac") %in% names(dt_med))) {
    p_abs <- p_abs +
      geom_ribbon(
      ...
      geom_line(
        data = dt_med[is.finite(median_frac)],
        aes(Temp, median_frac, colour = condition),
  ...
  p_abs <- p_abs +
    ...
    labs(
      ...
      subtitle = subtxt,
      x = "Temperature (°C)",
      y = "Fraction vs lowest temperature, Track B raw-LFQ"
    )
  
  if (show_fit_curve && nrow(dt_fit) > 0) {
    p_abs <- p_abs +
      geom_line(
    ...
    if (nrow(plot_dt) > 0) {
      ...
      p_scatter <- p_scatter +
        ...
        labs(
          ...
          subtitle = "Track A is prioritized; Track B raw-LFQ deltaTm is supportive",
          x = x_label,
          y = y_label
        )
