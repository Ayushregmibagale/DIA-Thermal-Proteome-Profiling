...
#############################################################################################################
#
# CETSA-MS SILAC TARGET CLASSIFICATION SCRIPT  v2.1
#
# PURPOSE
# -------
# Classify a requested target list into:
...
#   - not detected
#
# This version is aligned with the v2.1 CETSA-MS SILAC analysis pipeline:
#   - Track A SILAC ratio is primary evidence.
#   - Track B deltaTm / LFQ melt-curve evidence is secondary/supportive.
#   - No assumption that Track B should outrank Track A.
#
# INPUTS EXPECTED
# ---------------
# Required, from main CETSA SILAC pipeline output directory:
...
#
# Track B supportive/fallback:
#   deltaTm_mean, deltaTm_mean_curve, deltaAUC_mean, and Track B fraction-ratio evidence
#
# P-value handling:
...
# Track B thresholds: deltaTm / deltaAUC
strong_deltaTm_threshold   <- 2.0
weak_deltaTm_threshold     <- 1.0
strong_deltaAUC_threshold  <- 1.5
weak_deltaAUC_threshold    <- 0.5
...
# Track B MaxLFQ-derived log2 ratio thresholds
strong_log2ratio_B_threshold <- 0.35
weak_log2ratio_B_threshold   <- 0.20
destab_log2ratio_B_threshold <- -0.20
...
.sentinel <- "CETSA_SILAC_hit_table.tsv.gz"
.candidates <- c("CETSA_SILAC_output_v2_1", "CETSA_SILAC_output_v2", "CETSA_SILAC_output")
...
# Detect v2.1 / v2 / v1 column names
...
msg(sprintf(
  "Hit table columns: Track A score = %s | Track A signed = %s | Track B score = %s | p-value = %s",
  ifelse(is.na(score_col_A), "absent", score_col_A),
...
heat_source_label <- if (nrow(ratio_temp_dt) > 0 && "mean_log2_ratio" %in% names(ratio_temp_dt)) "Track A" else "Track B"
...
# If old hit table lacks signed Track A score, reconstruct sign from temperature-shape if available.
targets_master[!is.finite(score_A_signed) & is.finite(score_A_abs) & is.finite(max_ratio) & is.finite(min_ratio),
...
# Direction evidence separated by source.
...
# Require some temperature-shape consistency for ratio-only destabilization if signed peak is unavailable.
...
# Track A takes priority. Track B is used only when Track A is absent/weak or to support the same direction.
targets_master[, target_class := fcase(
  !detected,
  "not detected",
...
# Conflict labels are informative when Track A and Track B disagree.
...
build_reason_row <- function(row) {
  ...
  parts <- c(
    ...
    paste0("TrackB_deltaTm=", fmt(row$deltaTm_mean, 2)),
    paste0("TrackB_deltaTm_curve=", fmt(row$deltaTm_mean_curve, 2)),
    paste0("TrackB_deltaAUC=", fmt(row$deltaAUC_mean, 3)),
    paste0("TrackB_max_ratio=", fmt(row$max_ratio, 3)),
    paste0("TrackB_min_ratio=", fmt(row$min_ratio, 3)),
...
# Evidence score is Track A-first, not an unweighted mixture.
...
# Prefer Track A signed effect on x and Track A magnitude on y when present.
...
if (!is.null(y_col) && !is.na(y_col) && y_col %in% names(targets_master) && x_col %in% names(targets_master)) {
  ...
  p_scatter <- ggplot(
    ...
    labs(
      ...
      subtitle = "Track A SILAC ratio is prioritized; Track B deltaTm is supportive/fallback",
      x = x_label,
      y = y_label
    )
...
msg("\nEvidence conflicts, Track A vs Track B:")
